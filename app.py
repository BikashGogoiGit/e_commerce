...
def francise_login_action():
    if request.method == 'POST':
        bussiness = request.form['bussiness_type']
        unique = request.form['unique_id']
        name2 = request.form['name']
        password2 = request.form['password']
        cnx = mysql.connector.connect(user='root', database='e_commerce')
        cursor = cnx.cursor()
        query = f"SELECT name,password,bussiness_type from register where unique_id='{unique}'"
        cursor.execute(query)
        for (name, password,bussiness_type) in cursor:
            name1 = name
            password1 = password
            bussiness1 = bussiness_type
            if name1 == name2 and password1==password2:
                if bussiness1 == bussiness:
...
def francise_card_action():
    if request.method == 'POST':
        ...
        cnx = mysql.connector.connect(user='root', database='e_commerce')
        cursor = cnx.cursor()
        query = f"SELECT * from card_details where type_of_bussiness = 'francise' AND name_of_bussiness = '{francise_name}'"
        cursor.execute(query)
        for (unique_id,name_of_bussiness,
             type_of_bussiness,company,used_by,package) in cursor:
            unique_id = unique_id
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            add_data = ("INSERT INTO create_card1"
                        "(name_of_bussiness,package, company,logo,unique_id,activation_code)"
                        "VALUES(%s,%s,%s,%s,%s)")
            data = (name_of_bussiness,package,company,filename,unique_id,activation_code)
...
def personael_details_action():
    if request.method == 'POST':
       ...
       "(name_of_bussiness,first_name, last_name, designation,email_id,address,est_date,website,about,phone,Alt_phone,whatsapp,latitude,longitude,unique_id)"
       "VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)")
       data = (name_of_bussiness,first_name,last_name,position,
...
def social_link_action():
    if request.method == 'POST':
        ...
        add_data = ("INSERT INTO social_link"
         "(name_of_bussiness,unique_id,facebook, instagram,youtube)"
         "VALUES(%s,%s,%s,%s,%s)")
        data = (name_of_bussiness,unique_id,facebook,instagram,youtube)
        cursor.execute(add_data,data)
        cnx.commit()
        cursor.close()
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            add_data = ("INSERT INTO ecommerce"
                        "(name_of_bussiness,unique_id,product_id,product_name,mrp,selling_price,product_tax,prd_image)"
                        "VALUES(%s,%s,%s,%s,%s,%s,%s,%s)")
            data = (name_of_bussiness,unique_id,product_id,product_name,mrp,selling_price,product_tax,filename)
            cursor.execute(add_data,data)
            cnx.commit()
            cursor.close()
            cnx.close()
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            add_data = ("INSERT INTO image"
                        "(name_of_bussiness,img,unique_id)"
                        "VALUES(%s,%s,%s)")
            data = (name_of_bussiness,filename,unique_id)
            cursor.execute(add_data,data)
            cnx.commit()
            cursor.close()
            cnx.close()
...
def payable_amount():
    if request.method == "POST":
        ...
        cnx = mysql.connector.connect(user='root', database='e_commerce')
        cursor2 = cnx.cursor()
        query2 = f"SELECT product_name, selling_price, product_tax FROM ecommerce where unique_id='{prd_id}'"
        cursor2.execute(query2)
